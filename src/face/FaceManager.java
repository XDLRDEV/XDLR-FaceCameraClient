...
public class FaceManager {
    ...
    public FaceManager(FaceLoginInfo faceLoginInfo) {
        ...
        tag = "FaceManager " + faceLoginInfo.faceDeviceId + " " + faceLoginInfo.ip + " || ";
            ...
            if (NVSSDK.WCM_LOGON_NOTIFY == iMsgType) {    //设备登录消息
                ...
                if (NVSSDK.VCA_SUSPEND_STATUS_PAUSE == tParam.iStatus) {
                    if (NVSSDK.VCA_SUSPEND_RESULT_SUCCESS == tParam.iResult) {
                        System.out.println(tag + "智能分析暂停成功，1：继续添加，其他：退出添加");
                        logger.error(tag + "智能分析暂停成功");
                        if (snapNotifyListener != null) {
                            // 准备添加人脸底图
                            snapNotifyListener.vcaSuspendSucceed(FaceManager.this);
                        }
                    } else {
                        logger.info(tag + "智能分析暂停失败");
                        System.out.println(tag + "智能分析暂停失败，输入任意数字退出添加.");
                    }
                }
            }
        }
...
//        tInfo.tFaceLib.cName = String.valueOf(new Random().nextInt()+100).getBytes();
    ...
    int FacePictureQuery(int _iPageNo) {
        ...
        tInfo.cBirthEnd = "2018-10-31".getBytes();    //结束出生日期
        //end
        //非必需字段
    ...
    //人脸底图添加
    public int FacePictureAdd(String certNum, String negativePicturePath) {
        //选择人脸库，默认第一个库
        if (m_iLibKey <= 0) {
...
//        tInfo.tFace.cCertNum = IdUtil.generateId().getBytes();
        //end
//        tInfo.tFace.lOptType = 1;       //1添加    2修改
//        tInfo.tFace.cLibUUID = "".getBytes();
